...
export async function POST(request) {
  try {
    if (!process.env.ADMIN_PASSWORD || !process.env.ADMIN_SESSION_SECRET || !process.env.ADMIN_TOTP_SECRET) {
    ...
    if (!code) {
      ...
      response.cookies.set(ADMIN_PENDING_COOKIE, makeSignedToken(PENDING_AGE), {
        httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: PENDING_AGE,
      });
      return response;
    }

    if (!isAdminTwoFactorPending(request) || !verifyTotp(process.env.ADMIN_TOTP_SECRET, code)) {
      await pool.query("INSERT INTO admin_login_attempts (ip_hash) VALUES ($1)", [ipHash]);
