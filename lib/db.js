...
export async function ensureVolunteerTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS volunteer_applications (
      id BIGSERIAL PRIMARY KEY, full_name TEXT NOT NULL, age INTEGER, date_of_birth DATE, nationality TEXT, gender TEXT, faith TEXT, email TEXT NOT NULL, phone TEXT NOT NULL,
      ...
      emergency_name TEXT NOT NULL, emergency_phone TEXT NOT NULL, membership_fee_acknowledged BOOLEAN NOT NULL DEFAULT FALSE, consent BOOLEAN NOT NULL DEFAULT FALSE,
  ...
  await pool.query(`ALTER TABLE volunteer_applications
    ...
    ADD COLUMN IF NOT EXISTS membership_fee_acknowledged BOOLEAN NOT NULL DEFAULT FALSE,
...
export async function ensureNewsTable() {
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS news_articles_published_idx ON news_articles(status, published_at DESC)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS news_articles_category_idx ON news_articles(category)`);
}

export async function ensureEventsTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS vsi_events (
      id BIGSERIAL PRIMARY KEY,
      title TEXT NOT NULL,
      slug TEXT NOT NULL UNIQUE,
      category TEXT NOT NULL,
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS vsi_events_public_idx ON vsi_events(status, event_date ASC, start_time ASC)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS vsi_events_category_idx ON vsi_events(category)`);
}
