      ...
      <section className="news-list section-shell">
        {updates.map(([label, title, description, img], index) => (
          <article className={`news-feature ${index % 2 ? "reverse" : ""}`} key={title}>
            ...
            <div className="news-feature-copy"><span className="update-label">{label}</span><h2>{title}</h2><p>{description}</p><a className="text-link" href="/story">Explore VSI <span aria-hidden="true">→</span></a></div>
          </article>
        ))}
      </section>
