    ...
    <main>
      ...
      <section className="work" id="work">
        ...
        <div className="section-shell">
          <div className="focus-story-desktop" aria-label="VSI focus areas">
            <div className="focus-story-heading">
              <p className="kicker">WHERE WE FOCUS</p>
              <h2>Turning young people's energy into meaningful action.</h2>
              <p>Scroll through the areas where VSI creates space for young people to lead, serve, learn and shape change.</p>
            </div>
            <div className="focus-story-panels">
              {focusAreas.map(([n,t,d,img], index) => (
                <article className="focus-story-panel" key={t} style={{"--panel-index": index}}>
                  <div className="focus-story-image"><Image src={img} alt={`${t} programme`} fill sizes="(min-width: 1024px) 55vw" /></div>
          ...
          <div className="focus-grid focus-grid-mobile">
            {focusAreas.map(([n,t,d,img])=><article className="programme-card" key={t}><div className="programme-image"><Image src={img} alt={`${t} programme`} fill sizes="(max-width: 900px) 100vw, 33vw" /></div><div className="programme-content"><span>{n}</span><h3>{t}</h3><p>{d}</p></div></article>)}
          </div>
