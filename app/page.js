...
export default function Home() {
  return (
    <main>
      ...
      <section className="work section-shell" id="work"><div className="section-heading-row"><div><p className="kicker">WHAT WE DO</p><h2>Work that helps young people lead, serve and shape change.</h2></div></div><div className="focus-grid">{focusAreas.map(([n,t,d,img])=><article className="programme-card" key={t}><div className="programme-image"><Image src={img} alt={`${t} programme`} fill sizes="(max-width: 900px) 100vw, 33vw" /></div><div className="programme-content"><span>{n}</span><h3>{t}</h3><p>{d}</p></div></article>)}</div><div style={{display:"flex",justifyContent:"flex-end",marginTop:38}}><a className="text-link" href="https://web.facebook.com/vsizambia" target="_blank" rel="noreferrer">View all news on Facebook <span>↗</span></a></div></section>
