...
export async function GET(request){
 ...
 const [payments,donations,otherPayments,spending,volunteers,summary]=await Promise.all([
  ...
  pool.query(`SELECT o.*,v.full_name,v.volunteer_id AS vsi_id FROM volunteer_other_payments o JOIN volunteer_applications v ON v.id=o.volunteer_id ${volunteerId?"WHERE o.volunteer_id=$1":""} ORDER BY o.payment_date DESC,o.id DESC`,params),
...
export async function POST(request){
 ...
 if(b.type==="other_payment"){
  const volunteerId=Number(b.volunteerId),amount=Number(b.amount||0);if(!volunteerId||amount<=0||!String(b.description||"").trim())return NextResponse.json({error:"Volunteer, description and amount are required."},{status:400});
  const r=await pool.query(`INSERT INTO volunteer_other_payments(volunteer_id,description,amount,payment_date,payment_method,reference,notes) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *`,[volunteerId,String(b.description).trim(),amount,b.paymentDate||new Date().toISOString().slice(0,10),b.paymentMethod||null,b.reference||null,b.notes||null]);return NextResponse.json({otherPayment:r.rows[0]});
 }
 if(b.type==="spending"){
...
export async function PATCH(request){
 ...
 if(b.type==="other_payment"){
  const volunteerId=Number(b.volunteerId),amount=Number(b.amount||0);if(!volunteerId||amount<=0||!String(b.description||"").trim())return NextResponse.json({error:"Volunteer, description and amount are required."},{status:400});
  const r=await pool.query(`UPDATE volunteer_other_payments SET volunteer_id=$1,description=$2,amount=$3,payment_date=$4,payment_method=$5,reference=$6,notes=$7,updated_at=NOW() WHERE id=$8 RETURNING *`,[volunteerId,String(b.description).trim(),amount,b.paymentDate||new Date().toISOString().slice(0,10),b.paymentMethod||null,b.reference||null,b.notes||null,id]);if(!r.rowCount)return NextResponse.json({error:"Other payment record not found."},{status:404});return NextResponse.json({otherPayment:r.rows[0]});
 }
 if(b.type==="spending"){
  const amount=Number(b.amount||0);if(amount<=0||!String(b.category||"").trim()||!String(b.description||"").trim())return NextResponse.json({error:"Category, description and amount are required."},{status:400});
...
export async function DELETE(request){
 ...
 const table=type==="membership"?"volunteer_membership_payments":type==="donation"?"volunteer_donations":type==="other_payment"?"volunteer_other_payments":type==="spending"?"volunteer_finance_spending":null;if(!table)return NextResponse.json({error:"Unknown finance record type."},{status:400});
