...
const modules=[
  ...
  {eyebrow:"EVENTS",title:"Events",text:"Create and manage VSI events, speakers and event information.",href:"/admin/events",icon:"E"},
  {eyebrow:"EVENTS",title:"Event Registrations",text:"Review and manage registrations for VSI events.",href:"/admin/event-registrations",icon:"R"},
  {eyebrow:"DELIVERY",title:"Activity Register",text:"Review, verify and manage volunteer activity submissions.",href:"/admin/activities",icon:"A"},
  ...
  {eyebrow:"PEOPLE",title:"Performance",text:"Review volunteer performance and participation information.",href:"/admin/performance",icon:"P"},
  {eyebrow:"SECURITY",title:"Authenticator",text:"Manage the protected administrator authenticator setup.",href:"/admin/authenticator",icon:"2"}
];
...
export default function AdminDashboard(){
  ...
  return <><SiteHeader ctaLabel="Public site" ctaHref="/"/><main className={styles.page}><div className={styles.shell}><header className={styles.topbar}><div><div className={styles.brand}>VSI <span>ADMINISTRATION</span></div><p>Welcome to the VSI administration centre.</p></div><div className={styles.actions}><span className={styles.connected}><i/> Secure session</span><button onClick={logout}>Log out ↪</button></div></header><section className={styles.hero}><div><p className="kicker light">VISIONARY STUDENTS INITIATIVE</p><h1>What would you like to manage?</h1><p>Choose an administration area below. Your existing tools remain separate, but you can now reach everything from one central dashboard.</p></div><div className={styles.heroMark}>VSI</div></section><section className={styles.grid}>{modules.map((item)=><a className={`${styles.card} ${item.featured?styles.featured:""}`} href={item.href} key={item.href}><div className={styles.cardTop}><span className={styles.icon}>{item.icon}</span><span className={styles.eyebrow}>{item.eyebrow}</span></div><h2>{item.title}</h2><p>{item.text}</p><span className={styles.open}>Open {item.title} <b>→</b></span></a>)}</section></div></main><SiteFooter/></>;
}
