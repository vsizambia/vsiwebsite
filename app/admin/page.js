...
const modules=[
  {title:"Volunteer Management",text:"Review applications, manage the volunteer register, assignments and profiles.",href:"/admin/volunteers",featured:true},
  {title:"News Publishing",text:"Create, edit, publish and manage VSI news stories and media.",href:"/admin/news"},
  {title:"Events",text:"Manage VSI events and event registrations.",href:"/admin/events",events:true},
  {title:"Master Activity Catalogue",text:"Manage the approved Directorate → Project → Activity library.",href:"/admin/activity-catalogue"}
];
...
export default function AdminDashboard(){
  ...
  return <><SiteHeader ctaLabel="Public site" ctaHref="/"/><main className={styles.page}><div className={styles.shell}><header className={styles.topbar}><div></div><div className={styles.actions}><span className={styles.connected}><i/> Secure session</span><button onClick={logout}>Log out ↪</button></div></header><section className={styles.hero}><div className={styles.heroContent}><h1>VSI Administration Control Centre</h1><p>Manage VSI volunteers, readiness, deployment and recruitment across programmes and activities.</p></div></section><section className={styles.grid}>{modules.map((item)=><div className={`${styles.card} ${item.featured?styles.featured:""}`} key={item.href}>{item.events?<><h2>{item.title}</h2><p>{item.text}</p><div className={styles.subLinks}><Link href="/admin/events">Manage Events</Link><Link href="/admin/event-registrations">Event Registrations</Link></div></>:<Link className={styles.cardLink} href={item.href}><h2>{item.title}</h2><p>{item.text}</p><span className={styles.open}>Open {item.title} <b>→</b></span></Link>}</div>)}</section></div></main><SiteFooter/></>;
}
