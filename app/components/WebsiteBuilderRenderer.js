...
export default function WebsiteBuilderRenderer({page}) {
 return <main className="website-builder-page">{page.published_content.map((section,index)=>{const widths=layoutColumns[section.layout]||layoutColumns["100"];const style={"--builder-bg":section.bg||"#ffffff","--builder-color":section.color||"#173b58"};const columns=Array.isArray(section.columns)?section.columns:[{elements:[section]}];return <section className={`builder-section ${String(section.type||"Section").toLowerCase().replace(/\s+/g,"-")}`} style={style} key={section.id||index}><div className="builder-shell"><div className="builder-columns">{columns.map((column,columnIndex)=><div className="builder-column" style={{width:widths[columnIndex]||`${100/columns.length}%`}} key={column.id||columnIndex}>{(Array.isArray(column.elements)?column.elements:[]).map((element,elementIndex)=><ElementContent element={element} key={element.id||elementIndex}/>)}</div>)}</div></div></section>;})}</main>;
}
